...
def two(input):
	...
	if(input.count("bert") >= 2):
		return input[input.find("bert") + 4 :input.rfind("bert")]
	else:
		return ""
...
def four(arg1):
	...
	while(i < len(arg1)):
		if (arg1[i] != " "):
			currentSum += int(arg1[i])
		
			
		if (arg1[i] == " ") or (i+1 == len(arg1)):
...
def five(input1):
	li = list(input1.split(","))
	names = []
	
	i = 0
	while (i < len(li)):
		if li[i] == "False":
			if(li[i-2] not in names):
				names.append(li[i-2])
		i += 1
...
def six(input1):
	if ("c" in input1):
		if("ei" or "ie" in input1):
			cIndex = input1.find("c")
			if (cIndex < input1.find("ei")):
				return True 

			elif(cIndex < input1.find("ie")):
				return False
